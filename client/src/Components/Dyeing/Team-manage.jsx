...
const dyeingMembers = teamMembers.filter((m) => m.Dept === "Dyeing");
  ...
  <div className="bg-zinc-900 p-4 rounded shadow">
    ...
    <p className="text-xl font-bold">{dyeingMembers.length}</p>
       <p className="text-gray-400">Size of the Dyeing team</p>
  </div>
  <div className="bg-zinc-900 p-4 rounded shadow">
    <p className="text-gray-400">Active</p>
    <p className="text-xl font-bold">
      {dyeingMembers.filter((m) => m.Status === "Active").length}
    </p>
  ...
  <div className="bg-zinc-900 p-4 rounded shadow">
    ...
    <p className="text-xl font-bold">
      {dyeingMembers.filter((m) => m.Status === "In-Active").length}
    </p>
     <p className="text-gray-400">Number of In-Active Employees</p>
        ...
        <section className="bg-zinc-900 p-6 rounded shadow">
          ...
          <table className="w-full text-left text-sm">
           ...
           <tbody>
            {teamMembers
              .filter((member) => member.Dept === 'Dyeing')
              .map((member, idx) => (
